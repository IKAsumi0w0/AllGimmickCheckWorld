...
[CustomEditor(typeof(Mentaiko_Manager))]
public class Mentaiko_ManagerEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        var script = (Mentaiko_Manager)target;

        EditorGUILayout.Space(8);
        EditorGUILayout.LabelField("Mentaiko Pool Generator (Prefab)", EditorStyles.boldLabel);
    ...
    static void GenerateFromPrefab(Mentaiko_Manager script, GameObject prefab, int count)
    {
        var parent = script._pool;
        if (parent == null || prefab == null)
        {
            Debug.LogError("[Mentaiko_ManagerEditor] pool または prefab が未設定です。");
            return;
        }
        ...
        // 生成
        var list = new System.Collections.Generic.List<Mentaiko_Pickup>(count);
        for (var i = 0; i < count; i++)
        {
            ...
            Undo.RegisterCreatedObjectUndo(clone, "Create Mentaiko");
            clone.name = prefab.name + "_Copy_" + (i + 1);

            var pickup = clone.GetComponent<Mentaiko_Pickup>();
            if (pickup != null) list.Add(pickup);
            else Debug.LogWarning($"{clone.name} に Mentaiko_Pickup が見つかりません。");
        }
        ...
        Debug.Log($"Mentaiko を {list.Count} 個生成し、_objs に割り当てました。");
    }
}
